...
import { changeStoryCategoryStatus, getStoryCategoryLogs } from "../../Apis";
...
export default function StoryCategoryLog() {
  const {
    perPage,
    setPerPage,
    status,
  ...
  const {
    ...
    data,
    refetch,
  } = useFetchData("story_categories", getStoryCategoryLogs, [
    perPage,
    search_string,
    status,
    from,
  ...
  const { mutate, isLoading: loadingStatus } = useMutation(
    (id) => changeStoryCategoryStatus(id),
    {
      onSuccess: (res) => {
        refetch();
              ...
              <div className="card-dashboard">
                <div className="row">
                  <div className="col-lg-6 mt-2">
                    <h5 className="main-heading">Story Category Management</h5>
                  </div>
                  <div className="col-lg-6 text-right mt-2">
                    <Link
                      to="/story-category/add"
                ...
                <Table
                  headings={["NAME", "NUMBER OF STORIES", "STATUS", "ACTION"]}
                  perPage={perPage}
                    ...
                    {data?.data?.logs?.docs?.map((log) => (
                      <tr>
                        ...
                        <td>
                          <TableMenu
                            disable_view
                            edit_link={`/story-category/edit/${log?._id}`}
                            enable_edit
                            actionFunciton={() => {
                              Confirmation(
